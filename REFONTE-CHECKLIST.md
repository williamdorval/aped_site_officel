...
| 5 | **Le garde-fou du prix ne voyait pas la grille de prix** (défaut 29) | *(ce commit)* | Cherchait `PRICING`, la variable s'appelle `BAREME`. Les cinq paliers sont maintenant relevés et nommés. Prix injecté → sortie **1** ; `BAREME` renommé → sortie **2** en disant que c'est la sonde qui a dérivé. Fichiers identiques à HEAD |
