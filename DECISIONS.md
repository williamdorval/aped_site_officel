...
| **Les douze secteurs, chacun son identité — 2026-08-01** | 22 | 1 383 |
| **De 7/10 à 10/10 — 2026-08-01, seconde passe** | 17 | 1 384 |
| **UN ÉCRAN, PAS UN SITE — 2026-08-01, troisième passe** | 16 | 863 |
| **Le geste, et ce qu'il devient quand on le réduit — 2026-08-01** | 12 | 863 |
| **La passe finale — 2026-08-01, quatrième passe** | 21 | 924 |
...
| **Le chantier de conversion — 2026-08-02** | 45 | 4 259 |
...
d'ADEXWEB avec d'autres mots. Une section qui doit prouver qu'on sait
faire n'importe quel style prouvait exactement l'inverse.

| ID | Décision |
|---|---|
| D-663 | **Aucun site de secteur ne porte l'identité d'ADEXWEB**, et les interdits d'ADEXWEB — rayon 0, aucune ombre, aucun dégradé, aucun flou — **ne s'appliquent PLUS à l'intérieur d'un site de secteur.** Ils sont là pour montrer notre étendue. La clinique est le seul site à coins arrondis et à ombres douces, et c'est ce qui la rend reconnaissable à trois mètres. Le site ADEXWEB, lui, les garde tous |
...
| D-674 | **Le JavaScript est autorisé dans les sites de secteur. GSAP aussi, AUTO-HÉBERGÉ.** « Zéro script » était un plafond de qualité et il s'était vu : le surlignage cyan déclaré impossible, `timeline-scope` qui demande Chrome 116 quand `animation-timeline` demande 115. Ce qui ne bouge pas : zéro requête tierce — un CDN reste une requête tierce même quand il sert la bibliothèque qu'on a déjà sous la main — zéro erreur console, et **rien de nécessaire à la lecture ne dépend du script** : `demos-controle.mjs` recharge chaque page JavaScript coupé et exige 85 % du texte. Le site ADEXWEB, lui, garde tous ses interdits |
...
| D-714 | **Le plus gros chiffre du site était multiplié par cinq, et démenti par le document qu'on remet au même prospect.** `#comparatif` affichait « − 5 h 36 **par jour** » et disait les six tâches « **chronométrées** bout à bout ». La légende du guide gratuit, `adexweb-automatisation.html`, dit mot pour mot : « en **minutes par semaine**, pour l'entreprise au complet […] C'est un **ordre de grandeur d'atelier**, pas une donnée d'enquête. » Les six valeurs du site (100, 90, 90, 60, 60, 40) sont exactement celles de la table du guide. Q1 échoue deux fois — et la section promet elle-même « chaque chiffre porte sa source ». « par semaine » et « estimées » : aucune ligne de mise en page ne bouge, vérifié à 390 px |
| D-715 | **Le seuil « 0 échec de contraste » était mesuré sur la moitié du texte, par un outil qui ne pouvait pas échouer.** `theme-check.mjs` attendait 900 ms — `data-palier` arrive vers 1 520 ms — puis mesurait **sans jamais défiler**, en jetant tout élément au rectangle nul. Or sept sections et le pied portent `content-visibility: auto` : hors écran, leur rectangle EST nul. Mesuré le 2026-08-03 : **389 éléments de texte vus contre 747 après traversée, soit 47,9 % du site jamais soumis au seuil.** Et l'outil n'avait aucun `process.exit` : il imprimait un nombre que rien ne relisait. Trois correctifs : attendre `data-palier` au lieu de deviner, traverser avant de mesurer, et sortir en 1 sur faute. Il dit désormais « 0 échec **sur 740 éléments mesurés** » — un « 0 » sans son dénominateur ne prouve rien. Après réparation, le site tient toujours à **0**, mais cette fois c'est gagné |
| D-716 | **Le garde-fou du prix ne voyait pas la grille de prix.** `prix-check.mjs:123` cherchait `PRICING` et `base: { vitrine` ; la variable s'appelle `BAREME` (`js/main.js:29`). Il imprimait donc « GRILLE DE CALCUL (**jamais affichée telle quelle**) : 0 » — deux faussetés en un titre. Elle s'affiche : `main.js:2088-2101` pose ses bornes dans le `textContent` de `#priceLow` et `#priceHigh` à l'étape 8 de l'estimateur, et le visiteur lit « 2 500 $ à 5 000 $ », « 10 000 $ à 20 000 $ », « 40 000 $ et plus, sur devis ». L'outil relève désormais les cinq paliers, les nomme, et **s'ARRÊTE en 2** s'il n'en trouve aucun — un zéro veut dire que la sonde a dérivé, jamais que la grille a disparu. Il sort en 1 sur tout prix non autorisé. **Il ne tranche pas le barème** : `CLAUDE.md` interdit « aucun prix, nulle part » et D-353 le documente comme une décision assumée ; les deux ne peuvent pas être vrais, et l'arbitrage appartient au propriétaire. L'outil refuse seulement de rendre zéro en silence |
| D-717 | **La « VÉRIFICATION FINALE » n'avait aucun verdict.** `verif.mjs` mesurait les neuf seuils de `CLAUDE.md` puis imprimait un JSON que rien ne relisait — aucun `process.exit`. Douze seuils sont désormais tenus par une porte : LCP, CLS, i/s, requêtes tierces, anneaux de focus, piège de tabulation, focus de modale et son retour, orientation, débordement du 404, erreurs console des deux passes. **Deux mesures sont délibérément HORS de la porte**, et c'est écrit dans le code : `p404.erreurs` compte le HTTP 404 attendu du document lui-même, et `mouvementReduit.elementsInvisibles` compte les 12 `.mock` correctement cachés par `content-visibility: hidden` sous un `aria-hidden` — une porte qui échoue toujours ne garde rien. En cherchant à prouver que la porte mordait, on a trouvé un second trou : la passe de mouvement réduit n'écoutait que `pageerror`, jamais `console`, donc un `console.error` y passait inaperçu. Zéro arrêt au clavier relevé arrête l'outil en 2 : ça veut dire qu'il n'a rien parcouru, pas que tout va bien |
| D-736 | **Le blocage était dans le mauvais agenda, et rien ne le disait.** Le script s'exécute sous le compte de l'agence ; un « pas dispo » posé le 2026-08-06 sur le 12 août dans l'agenda PERSONNEL laissait les neuf créneaux du 12 août offerts sur le site. Aucun contrôle ne pouvait le voir : les 41 cas de `creneaux-check` ne posaient jamais qu'un seul agenda. `CALENDRIERS_EN_PLUS` lit autant d'agendas qu'on veut, avec les mêmes règles. Un agenda illisible **lève** au lieu de rendre une liste vide — « zéro occupation » voudrait dire « tout est libre », et c'est exactement comme ça qu'on donne deux fois le même rendez-vous. `initialiser()` les lit tous et nomme le fautif dans le journal. Une mutation a révélé au passage que le repli `CalendarApp` n'avait **jamais** été exercé : `faux-google` évalue désormais `Code.gs` une seconde fois sans le service avancé |
| D-737 | **La forme du classeur ne se prouvait pas depuis le dépôt.** Elle vit chez Google, et le partage Drive n'a pas atteint cette session — quatre requêtes, quarante minutes d'intervalle, rien. Sans preuve, la seule phrase possible était « c'est écrit dans le code, donc c'est bon », ce que ce projet refuse. `?action=diag` rend la structure — en-têtes dans l'ordre, largeurs, lignes figées, règles de couleur, formats — et le contenu des **seules** lignes portant un marqueur de `MARQUEURS_ESSAI`. La liste est en dur : aucun paramètre ne la choisit, donc une ligne de vrai client ne peut pas en sortir, même en la demandant. Elle rend aussi le verdict de l'injection, que `getValues()` ne peut pas donner : c'est `getFormulas()` non vide qui dit que Sheets a calculé |
| D-738 | **Le verrou tenait pendant les courriels.** Six envois simultanés, 18,4 s pour le dernier servi (2026-08-05). `LockService` protège deux courses — le numéro de ligne, qui se lit puis s'écrit, et la plage du calendrier, qui se vérifie puis se réserve. Les deux `MailApp.sendEmail`, à une ou deux secondes chacun, n'en sont pas et tenaient pourtant la file. Ils partent maintenant après la libération. **Et ça répare une seconde chose** : un envoi qui levait — quota atteint, adresse refusée — faisait échouer TOUTE la requête alors que la ligne était déjà écrite. Le visiteur lisait « le service a rencontré une erreur » sur une demande bel et bien reçue, et la renvoyait |
| D-739 | **Une politique de confidentialité, pas un bandeau de témoins.** Établi avant d'écrire une ligne : le site ne pose **aucun** témoin — zéro `document.cookie`, zéro requête tierce — et ses trois clés de stockage local (thème, popup déjà vu) ne portent ni identification, ni localisation, ni profilage. L'article 8.1 de la Loi 25 ne s'applique donc pas, et un bandeau annoncerait un suivi qui n'existe pas. Ce que la loi exige vraiment : une politique publiée (art. 8.2) et l'information **au moment** de la collecte (art. 8). Une PAGE, pas une modale — une politique doit se citer par son adresse, depuis un courriel ou une demande de la Commission. Elle dit la communication **hors Québec** vers Google, et elle **refuse de s'engager sur une durée fixe** faute de quoi la faire respecter ; le retrait en 30 jours, lui, tient. `confidentialite-check` cherche les deux façons de se tromper — promettre ce qu'on ne tient pas, taire ce qu'on doit dire — et ses six motifs sont éprouvés par mutation |
| D-740 | **Sous 24 rem, le calendrier n'est plus une grille, c'est une liste.** Sept colonnes dans un panneau de 288 px donnent des cases de 40 px : on passe le seuil AA de WCAG 2.5.8 (24 px), pas le AAA de 2.5.5 (44 px). Et ici, se tromper de case ne rate pas un clic — **ça réserve le mauvais jour**. La hauteur se force, la largeur non : il n'y a pas 307 px à distribuer. On a donc retiré la contrainte à sa racine. Une ligne pleine largeur par jour **offert**, 254 × 48 à 320 px, qui dit « sam 8 août » au lieu de « 8 ». Le seuil de bascule est **mesuré, pas calculé** : l'arithmétique donnait 348 px et oubliait le remplissage du panneau ; `pouce-check` a rendu 378 |
| D-741 | **Aucune requête n'attendait indéfiniment, et `fetch` n'a pas de délai maximum.** La porte des créneaux répond en 1,7 s de médiane — et une fois sur vingt-cinq, en 29,9 s. Une demi-minute de roue qui tourne sans un mot n'est pas de la lenteur : le visiteur conclut que c'est cassé et il ferme. Deux budgets, parce que les deux enjeux sont opposés : **8 s** pour l'affichage des créneaux, qui a un filet et qu'on remplace ; **25 s** pour un envoi, qui n'en a pas et qu'abandonner trop tôt ferait renvoyer. Le message du dépassement ne dit pas « panne » — il dit que la demande est peut-être passée, et que renvoyer est sans danger, ce qui est vrai depuis D-730 |
| D-742 | **L'outil des sept formulaires promettait un drapeau `--reel` qui n'existait pas** — un commentaire qui décrit une option absente fait croire que la chose a été faite. Ajouté, il a rendu **0 sur 8** contre le vrai service, avec zéro erreur de console : deux défauts d'instrument, tous deux invisibles. Le filtre de réponses était « l'url commence par l'adresse du service », or `/exec` répond par un 302 vers `googleusercontent.com` et c'est CETTE réponse qui porte le JSON. Et l'attente était de 2 500 ms fixes, calibrée sur un banc qui répond en 5 ms, contre un service qui met 2 à 8 s. Corrigé : on attend que l'écran change, et on **imprime le temps** — la mesure vaut le verdict |
