...
|---|---|---|---|---|---|---|---|---|
| **A70** | **la piste** | `main.js` `image` | `.svc-planche[data-svc-rail]` | position de défilement de la page, lue en direct | **aucune durée** — fonction pure de la position. La cible d'un cran est le CENTRE de l'élément, bornée à `[0, scrollWidth − clientWidth]` : c'est ce bornage qui cale le 01 à l'entrée et le 05 à la sortie | `html.js` + `≥ 48em` + `prefers-reduced-motion: no-preference`. **Ne tombe à aucun palier** | **V2** (le déplacement) arrêté d'un **V4** (le cran de 18 % à chaque bout) | **N1** |
| **A71** | **l'odomètre** | `main.js` `poserCompte` | `.svc-roul[data-svc-compte]` | changement de cran | `translateY` de 0 à −50 % en **240 ms `step-end`** — il roule d'un cran, il ne glisse pas | idem A70. Remplace à lui seul la liste et la jauge retirées le 2026-07-31 | **V4** | **N1** |
| **A72** | `svc-degage-nom` | `app.css` kf | `.svc-plan[data-vu] .svc-plan-nom::after` | `marquer` pose `data-vu` sur le courant **et le suivant** — sans quoi le chantier qui déborde s'affiche avec un trou à la place de son nom | **420 ms** `--e-drive` `forwards` | **`.svc-planche[data-degage]`**, posé par `marquer` : le voile ne peut pas exister sans son retrait (règle 0bis) | **V1** | N2 |
...
| **A78** | **le curseur avant / après** | `main.js` `avantApres` | `.ba-poignee` → `--ba-p` sur `.ba-scene` | `input` sur un `input[type=range]` natif | **aucune durée** — le rognage suit le doigt, la souris ou la touche. Flèches, Home et End viennent du natif ; `aria-valuetext` dit « N % de la version d'avant » | **repos à 50 %** : sans script la poignée est retirée et le partage reste moitié-moitié | **V1** — une arête franche qui balaye une forme déjà là | **N1** |
| **A79** | **la démonstration d'ouverture** | `main.js` `avantApres` | `--ba-p` de la première carte | `IntersectionObserver` à 0,5, **une seule fois**, 700 ms après l'entrée | **760 ms**, `1 − (1 − p)³`, de 100 vers 50 | le premier geste du visiteur l'annule ; tombe sous mouvement réduit **et** au palier 2 | **V1** | N2 |
| **A79b** | **`ba-defile`** | `app.css` kf | `.ba-scene[data-vif] .ba-page` | la scène entre dans le viewport | **34 s**, `--e-drive`, `infinite`, course **48cqw**, avec un palier d'arrêt à chaque bout | s'arrête **quatre fois** : hors viewport, au survol, sous mouvement réduit, aux paliers 1 et 2 | **V1**, sens bas | N3 |
| **A79c** | **`ap-rail`** | `app.css` kf | `.ba-scene[data-vif] .ap-rail` | idem | **17 s** `alternate`, `translateX` de 0 à −26cqw | idem A79b | mouvement **diégétique** — c'est le site montré qui bouge, pas le nôtre | N3 |
| **A79d** | **`ap-souder`** | `app.css` kf | `.ba-scene[data-vif] .ap-bloc-t::after` | idem | **34 s**, `scaleX` de 0 à 1, **repos à `scaleX(1)`** (règle 3) | idem A79b | **V3**, diégétique | N3 |
| **A79e** | **`v11-defile`** | `app.css` kf | `.v11-defile span` | permanente, **non conditionnée à la visibilité** | **16 s** `linear` `infinite` | tombe au palier **1**, au palier 2 et sous mouvement réduit — et le texte redevient alors **lisible à l'arrêt** (`padding-left: 0`) | diégétique | N3 |
