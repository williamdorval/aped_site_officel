...
| **Palette** | **sapin profond `#0b1712`** — le seul aplat de l'écran (cartouche, et couleur de `<html>` sous la photo) · **os `#e8e0cd`** — tout le texte, sans exception · **laiton `#a98b4f`** — le filet, la bordure du cartouche, l'aplat du bouton, **et la plaque du geste** · **voile froid `#122c3a`** posé à 26 % sur la photographie · **givre `#475b67`** — ce que la neige *devient* après traitement (mesuré, pas choisi) · noir du dégradé et des deux voiles `rgb(9,18,15)`. **Aucun orange, aucun rouge dans la palette dessinée.** Dans la photographie il en reste **61 pixels sur 1 296 000 — 0,047 ‰** : les carreaux allumés des pavillons, le plus saturé à `#392c24` (37 %). `saturate(.26)` les ramène à zéro et les deux planches sont indiscernables ; on garde `.34` parce que ces soixante-et-un pixels sont la seule chose de l'écran qui dise qu'il y a quelqu'un, et parce que `.26` rabat aussi la saturation de la neige de 31 à 27 % |
...
| **Formes** | **Deux rayons, et rien d'autre : 24 px** sur le cartouche (288 × 116), **20 px** sur le bouton (236 × 52) — le petit objet prend le plus petit rayon, c'est la règle. Le bouton fait 52 px de haut pour un rayon de 20 : il reste **12 px de segment droit** de chaque côté, donc ce n'est pas un stade. **La plaque du geste (506 × 16) est à angles vifs** : c'est une pièce de métal encastrée, pas un objet flottant ; les deux rayons restent réservés aux deux objets qui flottent. **Filets de 1 px** : le filet vertical, en **laiton plein** (à `.55` il tombait à 2,54:1 sur cette photo et s'effaçait ; plein il rend 5,52:1) ; le séparateur du haut et la bordure du cartouche restent à `rgba(169,139,79,.55)` — l'un est court, l'autre est posé sur un aplat. **Aucune ombre portée** — la profondeur vient de la photographie, qui en a trois plans. **Aucun flou, aucun `backdrop-filter`** : le cartouche se remplit à `rgba(11,23,18,.88)`, un aplat franc, et c'est ce qui lui donne ses 13,94:1 |
| **Traitement photo** | Sur le `<img>` : **`filter: saturate(.34) contrast(1.12) brightness(.60)`**, inchangé. Par-dessus, une nappe plate **`rgba(18,44,58,.26)`** sur toute la surface. Mesuré après traitement, composite en `canvas` au recouvrement exact : la neige du sol passe de `#69b8eb` (Y = 0,433, saturation 55 %) à **`#475b67`** (Y = 0,098, 31 %) — bleu > vert > rouge, donc **la neige reste bleue** ; la neige des branches tombe à `#263238` ; les troncs à `#263136`, froids eux aussi. **Le dégradé du bas** : `linear-gradient(180deg, rgba(9,18,15,0) 0, rgba(9,18,15,.22) 20%, rgba(9,18,15,.55) 42%, rgba(9,18,15,.82) 62%, rgba(9,18,15,.94) 84%, rgba(9,18,15,.97) 100%)`, posé de **y = 400 à y = 900**, inchangé. **UN VOILE EN HAUT, ET IL EST NOUVEAU** : `linear-gradient(180deg, rgba(9,18,15,.62) 0, rgba(9,18,15,.34) 45%, rgba(9,18,15,0) 100%)` sur **190 px**. La version précédente n'en avait pas et elle avait raison — son tiers supérieur était un mur d'épinettes plein à Y = 0,011. La nouvelle matière est l'inverse : un lacis de branches chargées de neige, clair et haché. Sans voile, le **pire pixel** sous le téléphone rend 3,82:1, sous la mention de démonstration 2,20:1, sous la nav 3,23:1 — une moyenne à 12,87 n'excuse pas une hampe posée sur une branche blanche. Avec : 7,30 · 4,20 · 6,21. Ce qu'il coûte est mesuré aussi : la luminance moyenne de la bande 0→400 tombe de 43,4 à 38,7 et son écart-type de 26,4 à 22,5 — **11 % de nuit en plus, 14 % de grain en moins**. La photographie reste intacte de **y = 190 à y = 400** |
...
au bas — mesuré sur la page rendue, la promesse rend **8,09:1** et la
marque **12,22:1**. L'objection du plan précédent (« son tiers bas est
...
| 3 | **Voile du haut** | 0 | 0 | 1440 × 190 | trois arrêts, `.62 → .34 → 0`. Nouveau — voir la DA |
| 4 | Dégradé du bas | 0 | 400 | 1440 × 500 | six arrêts, voir la DA |
| 5 | Téléphone `000 000-0000` | 72 | 48 | 100 × 14 | mono 11 px / 0,16 em, os 100 % — fond `#0f191b`, **13,59:1** (pire pixel 7,30) |
| 6 | Filet séparateur vertical | 186 | 51 | 1 × 10 | laiton `rgba(169,139,79,.55)` |
| 7 | `SITE DE DÉMONSTRATION` | 201 | 48 | 172 × 13 | mono 10 px / 0,22 em, **os à 72 %** — fond `#182224`, **7,05:1** (pire pixel 4,20). *62 % rendait 5,84:1 sur le mur d'épinettes de la version précédente ; sur ce lacis de branches il tombe à 3,58:1 au pire pixel même sous le voile. 72 % reste discret* |
| 8 | Nav, 4 items, écart 36 px | 783 | 48 | 585 × 17 | cormorant 600, 14 px / 0,26 em, os 100 % — fond `#1c2729`, **11,65:1** (pire pixel 6,21). Largeurs mesurées : 103 / 144 / 91 / 142. Le dernier item porte `margin-right:-0.26em` pour que l'encre s'aligne sur 1368, pas la chasse morte |
| 9 | Filet de laiton vertical | 72 | 436 | 1 × 412 | **laiton plein `#a98b4f`**, statique. **5,52:1** contre son fond `#10191a`. *Il portait le geste ; il ne le porte plus* |
| 10 | Surtitre `HAUTE-CÔTE-NORD, QUÉBEC` | 112 | 610 | 207 × 14 | mono 11 px / 0,22 em, **os à 76 %** — fond `#1b2628`, **7,39:1** (pire pixel 5,62). *En os, pas en laiton : le laiton tombe à 3,88:1 ici* |
| 11 | **La marque** | 112 | 643 | **516 × 25** d'encre | cormorant 600, **21 px**, interlignage 1,0, **chasse 0,44 em**, os 100 % — fond `#192324`, **12,22:1**. Boîte 112 / 645 / 515,7 × 51 : 21 px de texte, puis 30 px de `padding-bottom` qui réservent la plaque |
| 12 | **La plaque — LE GESTE** | 112 | 680 | **506,5 × 16** | angles vifs, laiton plein `#a98b4f`, **5,26:1** contre son fond `#141e1e` (seuil des non-textes : 3:1). Largeur = `calc(100% − var(--chasse))` sur `.marque` : **l'encre du nom au pixel, sans nombre magique**, et elle se recote seule quand la chasse change sous 1080 px. Course en creux derrière, `rgba(232,224,205,.12)`, même boîte. À t = 1500 ms : **309 px de laiton, x 112 → 420** |
| 13 | Promesse, ligne 1 | 112 | 714 | 350 × 24 | spectral italique 16 px, os à 76 % — fond `#111c1b`, **8,09:1** (pire pixel 7,77) |
...
| 16 | Cartouche « conditions » | 1080 | 732 | **288 × 116** | **rayon 24 px**, remplissage `rgba(11,23,18,.88)` sur un fond nu `#0f1918` — composite mesuré **`#0b1713`** — bordure 1 px `rgba(169,139,79,.55)`, marge intérieure **26 px sur les côtés, 22 px en haut, 23 px en bas**. os dessus : **13,94:1** |
| 16a | ↳ `CE MATIN À L'ANSE` | 1106 | 754 | 139 × 13 | mono 10 px / 0,22 em, **laiton `#a98b4f` — 5,66:1**, seul endroit où le laiton porte du texte : il est sur un aplat |
| 16b | ↳ `−21 °C · ANSE PRISE` | 1106 | 779 | 208 × 21 | cormorant 600 17 px / 0,20 em, os 100 % |
| 16c | ↳ `LEVER 7 H 12 — COUCHER 16 H 04` | 1106 | 812 | 222 × 13 | mono 10 px / 0,14 em, **os à 58 %** — 5,37:1 |
...
surtitre y mesure 7,39:1.

**Le rectangle x 0→1440 · y 190→400 est de la photographie pure** —
210 px, 23 % de l'écran. C'était 400 px et 44 % ; le voile du haut a
pris les 190 premiers. Rien n'y est POSÉ dans les deux cas : ce qui a
