...
| **Formes** | **Angles vifs partout, rayon 0.** Aucune ombre, aucun dégradé, aucun flou. Deux objets pleins seulement sur tout l'écran : la photographie et le rectangle cramoisi du bouton. Tout le reste est **du texte et des filets de 1 px**. Le cramoisi n'apparaît que **trois fois** : le bouton (masse), la lettrine (signature imprimée), le folio (marge) — plus l'arête du masque pendant la course |
...
| **Le geste et l'instant de capture** | **Un seul geste : le titre se découvre sous un masque qui balaye de gauche à droite**, `clip-path: inset(0 100% 0 0)` → `inset(0 0 0 0)`, **1100 ms**, `cubic-bezier(.19, 1, .22, 1)`, la **ligne 2 retardée de 180 ms**. Sur l'arête du masque, un **filet cramoisi de 1 px** de la hauteur de la ligne, qui suit la même courbe : *la réglure verticale de la grille, en train de se déplacer.* C'est pour ça que ce geste-là et pas un autre — le mouvement est fait de la matière de la page. **Instant de capture : t = 280 ms.** À cette image, ligne 1 découverte à **83 %** (l'arête cramoisie coupe entre le `t` et le `s` de `doigts,`), ligne 2 à **47 %** (l'arête tombe entre `une` et `lame`). **Deux arêtes cramoisies verticales, décalées, en pleine course**, sur un titre déjà lisible aux deux tiers. Le masque est un `clip-path`, jamais une opacité : ce qui est découvert est noir plein. `prefers-reduced-motion: reduce` → aucune animation, les deux lignes entières, aucune arête, aucune information perdue |
...
molle nulle part, et un seul rouge qui n'apparaît qu'à trois endroits —
le bouton, la lettrine, le folio. Les photographies sont en noir et
