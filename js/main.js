...
(function () {
  ...
  (function avantApres() {
    ...
    /* --- 3 · LA LOUPE ---  D-647
       LE CADRE AGRANDI EST LE MEME CADRE, DEPLACE.
       Une copie aurait double le markup, double les images servies,
       et laisse deux etats qui divergent des le premier glissement.
       On DEPLACE donc le noeud dans le dialogue et on le remet a sa
       place en fermant : la poignee, la course, la scene epinglee et
       la position ou le visiteur en etait continuent sans qu'une
       seule ligne soit reinitialisee. Le `ResizeObserver` pose sur la
       scene remesure la piste tout seul quand le cadre change de
       taille — c'est pour ca que tout est calcule en `cqw`.
       Ce qui remplace le cadre pendant son absence est un TROU de la
       meme hauteur : sans lui, la grille se referme et la page saute
       sous le visiteur, qui retrouverait sa section deplacee. */
    (function loupe() {
      var dlg = doc.getElementById("ba-loupe");
      var hote = dlg && $("[data-ba-hote]", dlg);
      if (!dlg || !hote || typeof dlg.showModal !== "function") return;
      var cadreOuvert = null;
      var trou = null;
      var declencheur = null;

      function ouvrir(cadre, bouton) {
        if (cadreOuvert) return;
        cadreOuvert = cadre;
        declencheur = bouton;
        trou = doc.createElement("div");
        trou.className = "ba-trou";
        trou.style.height = cadre.getBoundingClientRect().height + "px";
        cadre.parentNode.insertBefore(trou, cadre);
        hote.appendChild(cadre);
        dlg.showModal();
        var f = $(".loupe-fermer", dlg);
        if (f) f.focus();
      }

      function fermer() {
        if (!cadreOuvert) return;
        if (trou && trou.parentNode) {
          trou.parentNode.insertBefore(cadreOuvert, trou);
          trou.parentNode.removeChild(trou);
        }
        trou = null;
        var c = cadreOuvert;
        cadreOuvert = null;
        /* ON REVIENT EXACTEMENT OU ON ETAIT. Le dialogue a fige le
           defilement de la page pendant qu'il etait ouvert ; on rend
           le focus au bouton qui a ouvert, et le navigateur ramene
           la vue sur lui. */
        if (declencheur && doc.contains(declencheur)) declencheur.focus({ preventScroll: false });
        declencheur = null;
        /* La scene a change de taille deux fois en une image : on
           laisse le `ResizeObserver` faire sa passe avant de rendre
           la main. */
        if (c) requestAnimationFrame(function () { c.style.opacity = ""; });
      }

      var boutons = $$("[data-ba-ouvrir]");
      for (var i = 0; i < boutons.length; i++) {
        (function (b) {
          b.addEventListener("click", function () {
            var cadre = b.closest(".ba-cadre");
            if (cadre) ouvrir(cadre, b);
          });
        })(boutons[i]);
      }
      var fermeture = $("[data-ba-fermer]", dlg);
      if (fermeture) fermeture.addEventListener("click", function () { dlg.close(); });
      /* CLIC A L'EXTERIEUR : un `<dialog>` recoit le clic sur son
         fond, et le fond n'est pas un enfant. On ferme donc quand le
         point du clic tombe hors du rectangle du dialogue. */
      dlg.addEventListener("click", function (e) {
        var r = dlg.getBoundingClientRect();
        if (e.clientX < r.left || e.clientX > r.right || e.clientY < r.top || e.clientY > r.bottom) dlg.close();
      });
      /* `close` couvre Echap, le bouton et le clic dehors d'un coup :
         c'est le seul endroit ou remettre le cadre en place. */
      dlg.addEventListener("close", fermer);
    })();

    /* --- 4 · LA DEMONSTRATION D'OUVERTURE ---  D-534 */
