...
/* ---------- 1bis · LA LOUPE ----------  D-647
   Le cadre agrandi est le MEME noeud, deplace. On le prouve en
   mesurant que la scene grandit, que la course se recalcule, que la
   poignee glisse encore et que le defilement descend — puis qu'en
   fermant, le cadre est revenu a sa place et la page n'a pas bouge. */
{
  ...
  for (const id of CARTES) {
    ...
    const petite = await p.evaluate((i) => {
      const s = document.querySelector("#" + i + " .ba-scene");
      const v = document.querySelector("#" + i + " [data-ba-vitre]");
      return { scene: `${Math.round(s.clientWidth)}x${Math.round(s.clientHeight)}`, course: v.scrollHeight - v.clientHeight };
    }, id);
    const yPage = await p.evaluate(() => Math.round(window.scrollY));
    await p.click(`#${id} [data-ba-ouvrir]`);
    await p.waitForTimeout(900);
    await p.screenshot({ path: path.join(OUT, `loupe-${id}.png`) });
    ...
    const grande = await p.evaluate(() => {
      ...
      return {
        ouverte: d.open,
        cadreDedans: !!d.querySelector(".ba-cadre"),
        trouPose: !!document.querySelector(".ba-trou"),
        scene: `${Math.round(s.clientWidth)}x${Math.round(s.clientHeight)}`,
        course: v.scrollHeight - v.clientHeight
      };
    });

    /* La poignee glisse-t-elle EN GRAND ? On la glisse pour de vrai. */
    const bg = await p.evaluate(() => {
      const r = document.querySelector("#ba-loupe .ba-scene").getBoundingClientRect();
      return { x: r.left, y: r.top, w: r.width, h: r.height };
    });
    await p.mouse.move(bg.x + bg.w * 0.5, bg.y + bg.h * 0.5);
    await p.mouse.down();
    const posesG = [];
    for (const k of [0.25, 0.75]) {
      await p.mouse.move(bg.x + bg.w * k, bg.y + bg.h * 0.5, { steps: 8 });
      await p.waitForTimeout(140);
      posesG.push(await p.evaluate(() => Math.round(+getComputedStyle(document.querySelector("#ba-loupe .ba-scene")).getPropertyValue("--ba-p"))));
    }
    await p.mouse.up();

    /* Le defilement, en grand, sur toute sa course. */
    const fg = [];
    for (let i = 0; i < 5; i++) {
      await p.evaluate((f) => { const v = document.querySelector("#ba-loupe [data-ba-vitre]"); v.scrollTop = (v.scrollHeight - v.clientHeight) * f; }, i / 4);
      await p.waitForTimeout(450);
      const f = path.join(OUT, `loupe-defile-${id}-${i}.png`);
      await p.screenshot({ path: f });
      fg.push(f);
    }
    const ecG = [];
    for (let i = 1; i < fg.length; i++) ecG.push(+diffStats(lire(fg[i - 1]), lire(fg[i]), 8).pct.toFixed(2));
    ...
    const apres = await p.evaluate((i) => ({
      ...
      trouRetire: !document.querySelector(".ba-trou"),
      y: Math.round(window.scrollY)
    }), id);

    R.loupe[id] = {
      petite, grande,
      poigneeEnGrand: posesG,
      ecartsDefileEnGrand: ecG,
      ecartMinimalEnGrand: Math.min(...ecG),
      pageAvant: yPage,
      apresFermeture: apres
    };
  }
  await ctx.close();
}
